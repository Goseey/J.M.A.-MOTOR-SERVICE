...
/**
 * Persist car_registration on an already-created/updated row when the column
 * exists. Kept as a follow-up statement so the many INSERT/UPDATE branches above
 * stay untouched. Best-effort: a failure here never breaks the main write.
 * Always mirrors the value onto the returned row object so callers/UI see it.
 */
async function applyCarRegistration(fn, columnSupport, row, carRegistration) {
  if (!row) return row;
  const value = typeof carRegistration === 'string' && carRegistration.trim().length > 0
    ? carRegistration.trim()
    : null;
  if (columnSupport.hasCarRegistration) {
    try {
      await fn`UPDATE service_requests SET car_registration = ${value} WHERE id = ${row.id}`;
      row.car_registration = value;
    } catch (error) {
      console.error('[db] Failed to set car_registration:', error?.message || error);
      row.car_registration = null;
    }
  } else {
    row.car_registration = null;
  }
...
export async function insertServiceRequest(payload) {
  ...
  if (columnSupport.hasSource && columnSupport.hasCreatedByAdminId) {
    ...
    `;
    return applyCarRegistration(fn, columnSupport, rows[0], car_registration);
  }
...
export async function updateServiceRequest(id, payload) {
  ...
  if (columnSupport.hasAdminNote) {
    ...
    return applyCarRegistration(fn, columnSupport, rows[0] || null, car_registration);
  }
