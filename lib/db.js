...
export async function insertServiceRequest(payload) {
  ...
  if (columnSupport.hasSource) {
    const rows = await fn`
      ...
      RETURNING id, customer_name, phone, email, car_make_model, service_needed,
                preferred_date, message, selected_language, source, NULL::uuid AS created_by_admin_id, status, created_at, updated_at
    `;
    return rows[0];
  }
  ...
  const rows = await fn`
    ...
    RETURNING id, customer_name, phone, email, car_make_model, service_needed,
              preferred_date, message, selected_language, 'website'::text AS source, NULL::uuid AS created_by_admin_id, status, created_at, updated_at
  `;
  return rows[0];
}
...
export async function updateServiceRequest(id, payload) {
  ...
  const rows = await fn`
    ...
    WHERE id = ${id}
    RETURNING id, customer_name, phone, email, car_make_model, service_needed,
              preferred_date, message, selected_language, status, created_at, updated_at
  `;
