...
/**
 * Extract the client IP from a Headers-like object.
 *
 * SECURITY: prefer `x-real-ip`, which Vercel sets to the real client IP and a
 * client cannot forge. The first entry of `x-forwarded-for` is attacker-
 * controllable (Vercel appends the true IP after any client-supplied value), so
 * relying on it would let someone rotate the header to bypass the login
 * brute-force block and the public API rate limit. `x-forwarded-for` is only a
 * fallback for non-Vercel / local environments.
 */
