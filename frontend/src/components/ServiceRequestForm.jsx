...
export default function ServiceRequestForm() {
  ...
  const validate = () => {
    ...
    if (!form.phone.trim() || form.phone.trim().length < 5) e.phone = 'Please enter a contact phone number.';
    if (form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) e.email = 'Please enter a valid email address.';
    if (!form.car_make_model.trim()) e.car_make_model = 'Please tell us the car make and model.';
    if (!form.service_needed.trim()) e.service_needed = 'Please choose a service.';
    setErrors(e);
        ...
        {status === 'success' ? (
          ...
          >
            ...
            <Field
              ...
              required
              error={errors.car_make_model}
              testid="form-field-car"
            >
              <input
                type="text"
                value={form.car_make_model}
                onChange={update('car_make_model')}
                className={inputCls(errors.car_make_model)}
                placeholder="VW Golf 1.6 TDI, 2016"
            ...
            <Field
              ...
              testid="form-field-service"
            >
              <div className="relative">
                <select
                  value={form.service_needed}
                  onChange={update('service_needed')}
                  className={`${inputCls(errors.service_needed)} appearance-none pr-10 cursor-pointer`}
            ...
            <Field
              label="Message"
              hint="optional — describe the issue"
...
function Field({ label, hint, required, error, children, testid, className = '' }) {
  return (
